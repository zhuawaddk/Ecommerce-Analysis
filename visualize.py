...
# 剔除首尾不完整月份（订单量过低的噪声月，避免环比增长率失真）
df_monthly = df_monthly[df_monthly["order_count"] >= 100].reset_index(drop=True)
# 过滤后重算环比，避免被已剔除月份的小基数放大
df_monthly["mom_growth_pct"] = df_monthly["monthly_gmv"].pct_change() * 100
